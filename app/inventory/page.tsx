...
import { ChevronLeft, Plus, Search, Trash2, Edit2, X, Check, Package } from "lucide-react";
import { getAllInventory, saveInventoryItem, deleteInventoryItem, newInventoryId } from "@/lib/store";
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center gap-3 px-4 py-3.5"
          ...
          <Package size={16} color="#C8922A"/>
          <p className="font-bold text-sm flex-1" style={{ color:"#F2D080" }}>
        ...
        <div className="p-4 flex flex-col gap-3">
          {[
            ...
            <div key={f.key}>
              ...
              <input className="glass-input text-sm" type={f.type}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { key:"quantityDeployed",  label:"Deployed" },
              { key:"reorderThreshold",  label:"Reorder At" },
              { key:"unitCost",          label:"Unit Cost (KES)" },
            ].map(f => (
              <div key={f.key}>
                <label className="text-xs text-white/50 mb-1 block">{f.label}</label>
                <input className="glass-input text-sm" type="number" min={0}
                  value={Number(form[f.key as keyof typeof form] ?? 0)}
                  onChange={e => upd(f.key as keyof typeof form, parseFloat(e.target.value)||0)} />
              </div>
            ))}
          </div>
          <div>
...
export default function InventoryPage() {
  ...
  const [items, setItems] = useState<InventoryItem[]>(() => getAllInventory());
  ...
  return (
    <div className="min-h-screen page-enter">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs" style={{ color:"rgba(200,146,42,0.6)" }}>
                {items.length} items · {totalValue > 0 ? fmt(totalValue) + " est. value" : "no pricing"}
              </p>
            </div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-3">
        {filtered.length === 0 ? (
          <div className="glass-card p-10 flex flex-col items-center gap-4 text-center">
            ...
            {!search && (
              <button onClick={() => setModal({ ...BLANK })} className="gold-btn mt-2">
                Add First Item
              </button>
            )}
          </div>
              ...
              <div key={item.id} className="glass-card p-4 flex items-center gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {item.quantityDeployed > 0 && (
                      <span className="stat-pill text-xs">Deployed: {item.quantityDeployed}</span>
                    )}
                    {item.unitCost > 0 && (
                      <span className="stat-pill text-xs">{fmt(item.unitCost)} ea.</span>
                    )}
                  </div>
                </div>
      ...
      {modal && (
        <InventoryModal item={modal} onSave={handleSave} onClose={() => setModal(null)} />
      )}
    </div>
  );
}
