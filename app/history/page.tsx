...
export default function HistoryPage() {
  ...
  const [quotes, setQuotes] = useState<ManifestResult[]>(() => getAllQuotes());
            ...
            {["ALL","DRAFT","CONFIRMED","DEPLOYED","COMPLETED"].map(s => (
              <button key={s} onClick={() => setFilter(s)}
                ...
                style={{
                  ...
                  color: filter === s ? "#F2D080" : "rgba(255,255,255,0.45)",
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-3">
        {filtered.length === 0 ? (
          <div className="glass-card p-10 flex flex-col items-center gap-4 text-center">
            ...
            {!search && (
              <button onClick={() => router.push("/quote/new")} className="gold-btn mt-2">
                New Quote
              </button>
            )}
          </div>
          ...
          filtered.map(q => {
            const statusColor = STATUS_COLORS[q.status] ?? "#C8922A";
            const totalGuests = q.totalGuests;
            const totalTents  = q.totalTents;

            return (
              <div key={q.refNumber}
                className="glass-card p-4 flex items-center gap-4 group cursor-pointer"
                onClick={() => router.push(`/quote/${q.refNumber}`)}>
                <div className="w-1 h-14 rounded-full flex-shrink-0" style={{ background: statusColor }} />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs mb-2" style={{ color: "rgba(200,146,42,0.6)", fontFamily: "monospace" }}>
                    {q.refNumber}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <span className="stat-pill text-xs gap-1"><Users size={9}/>{totalGuests} guests</span>
                    <span className="stat-pill text-xs gap-1"><Tent size={9}/>{totalTents} tents</span>
                    <span className="stat-pill text-xs gap-1"><Calendar size={9}/>{q.config.days}d</span>
                    {q.config.location && (
                      <span className="stat-pill text-xs">{q.config.location.split(",")[0]}</span>
                    )}
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <button className="p-2 rounded-xl" style={{ color: "rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.06)" }}>
                    <Eye size={13}/>
                  </button>
